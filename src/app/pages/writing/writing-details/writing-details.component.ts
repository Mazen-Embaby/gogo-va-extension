...
@Component({
  ...
  templateUrl: './writing-details.component.html',
  styleUrl: './writing-details.component.scss',
})
export class WritingDetailsComponent implements OnInit {
  ...
  loadConversation(id: string) {
    if (!this.isLoading) {
      ...
      this.writingService.getConversationById(id).subscribe({
        next: async (res) => {
          if (res) {
  ...
  async sendMessage() {
    ...
    try {
      const streamText = await this.writingService.streamText(userInput);
