...
export class SummaryService  {
  ...
  private _conversationKey = 'summary';
  ...
  async createSummary (text: string, createSummarizeParams : CreateSummarizeParams, summarizeOptionParams: SummarizeOptionParams) {
    const available = (await self.ai.summarizer.capabilities()).available;
    let summarizer;
    if (available === 'no') {
      console.info('The Summarizer API is not usable.');
      return;
    }
    if (available === 'readily') {
      summarizer = await self.ai.summarizer.create(createSummarizeParams);
      const summary = await summarizer.summarize(text, summarizeOptionParams);
      return summary;
    } else {
      console.info('The Summarizer API can be used after the model is downloaded.');
      summarizer = await self.ai.summarizer.create(createSummarizeParams);
      return '';
    }
  }

  // async streamSummary (text: string) {
    async streamSummary (text: string, createSummarizeParams : CreateSummarizeParams, summarizeOptionParams?: SummarizeOptionParams) {

    const available = (await self.ai.summarizer.capabilities()).available;
    let summarizer;
    if (available === 'no') {
      console.info('The Summarizer API is not usable.');
      throw Error('The Summarizer API is not usable.');
    }
    if (available === 'readily') {
      console.info('a');

      summarizer = await self.ai.summarizer.create(createSummarizeParams);
      console.info('BBBBBBBBBBBBBB');

      const summary = await summarizer.summarizeStreaming(text, summarizeOptionParams);
      return summary;
    } else {
      console.info('The Summarizer API can be used after the model is downloaded.');
      summarizer = await self.ai.summarizer.create();
      throw Error('The Summarizer API can be used after the model is downloaded');
    }
  }
